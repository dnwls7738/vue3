...
<script>
import YoutubeSearchVue from '@/includes/YoutubeSearch.vue';
...
export default {
	...
	setup() {
		...
		const SearchYoutube = search => {
			...
			fetch(
				`https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=28&q=${search}&key=${process.env.VUE_APP_API}&type=video`,
				requestOptions,
			)
